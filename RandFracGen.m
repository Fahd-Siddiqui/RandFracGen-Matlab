...
% Generates random fractures
% Rotates them
% Selects the rotated fractures
% Ensures fractures of size 0 and size 1 elements are removed
% Rescales the problem to a given size
% Returns the location matrix with [X_Beg Y_Beg X_End Y_End Fractre_Number]
% N, Mx, My and tun are used to control the randomness and size of elements
% Size of final domain
%   Mx: X size of final domain
%   My: Y size of final domain
%   theta: Angle of rotation +ve values only
%   N: Size of unrotated Domain
%   tun: Distribution of fractures 9 for large fractures 2 for small fractures
%   plot: set to yes for plotting
% Calling example: [locationR, Dom]=RandFracGen(500,5,5,-pi/12,5,0);
...
for i=2:size(Tiles,1)
    ...
    for j=1:size(Tiles,2)
        if (j>2&&j<size(Tiles,2))&&((Tiles(i,j)==Tiles(i-1,j)||Tiles(i,j)==Tiles(i-1,j-1)||Tiles(i,j)==Tiles(i-1,j+1))&&Tiles(i,j)==false)
            Tiles(i,j)=~Tiles(i,j);
        end
    end
end
...
while i<=size(B,1)
    ...
    if norm(B(i,1:2)-B(i,3:4))==0||norm(B(i,1:2)-B(i,3:4))==1
        B(i,:)=[];
        i=i-1;
    end
    i=i+1;
...
r=abs((s)/sqrt(2));
rshift=r/sqrt(2);
BOX=[MPR(1)-rshift,MPR(2)-rshift;MPR(1)-rshift,MPR(2)+rshift;MPR(1)+rshift,MPR(2)+rshift;MPR(1)+rshift,MPR(2)-rshift;MPR(1)-rshift,MPR(2)-rshift];
...
i=1;
while i<=Dom.Nf
    lx_min=double(Dom.Frac(i).locR(:,2)>x_min).*double(Dom.Frac(i).locR(:,2)>x_min);
    lx_max=double(Dom.Frac(i).locR(:,2)<x_max).*double(Dom.Frac(i).locR(:,2)<x_max);
    ly_min=double(Dom.Frac(i).locR(:,1)>y_min).*double(Dom.Frac(i).locR(:,3)>y_min);
    ly_max=double(Dom.Frac(i).locR(:,1)<y_max).*double(Dom.Frac(i).locR(:,3)<y_max);
    F=lx_min.*lx_max.*ly_min.*ly_max;
    
    Dom.Frac(i).locR=Dom.Frac(i).locR.*F;
